...
门资产与 10 cm 预抓取对齐已经通过。继续按如下顺序：把预抓取偏移缩到 2–3 cm并验证碰撞余量；夹爪由独立 grasp state 控制；加入腕部/门把手相对位姿和接触/力信息；先验证闭合与抓持，再训练把手旋转，最后训练保持连接时的本体后退。不要同时打开全部接触自由度。

## 8. 当前已知限制

- 目前已有 flat plane 上最大 70 cm 的平面目标、前方 -18 cm 低位目标，以及铰接门场景和把手预抓取对齐；尚未完成夹持、旋转把手、拉门、视觉或触觉。
- 夹爪只保持，不参与学习。
