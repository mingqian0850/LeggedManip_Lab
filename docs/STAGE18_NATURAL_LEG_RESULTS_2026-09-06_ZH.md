...
但 Stage10 极低目标仅 112/128 存活，16 次均为 `link2`/`lidar_link` 接触。固定镜像投影限制了极低位所需的前后腿差动和机身俯仰，因此当前版本不能替换 Stage18 默认模型。

下一实验是高度自适应投影：普通目标使用 25% 非对称残差，目标高度偏移低于约 -14 cm 时恢复 100% 非对称自由度，在 -8 cm 到 -14 cm 之间连续插值。验收仍需普通动态基准、Stage10 三 seed 和近距离视频全部通过。
