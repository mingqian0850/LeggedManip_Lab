...
下一阶段确实是训练，但不能直接启动 4096 个环境长时间训练。正确顺序是：
...
前五步的目的是保证训练问题本身正确，第六步才是正式学习策略。否则 PPO
可能只是学会补偿错误坐标、错误轮子方向或错误 TCP，而不是学会正确的全身
协调。

当前进度：第 4 步中的固定本体、重力补偿微分 IK 已通过 64 个 RTX 4090
并行环境测试。五组正负 6D 目标的最差位置 RMS 为 2.19 mm，最差姿态 RMS
为 0.24 度，最大终端抖动为 1.63 mm。下一项是固定世界目标在浮动本体运动中
保持不变，并验证轮子方向和底盘执行层；尚未进入 PPO 长训练。
...
| 2. 验证轮子方向和有效半径 | 逐轮检查正负方向、关节顺序和承重后的滚动半径 | 防止轮子互相对抗，并保证轮速与底盘速度一致 |
| 3. 实现固定世界坐标目标 | 每个环境保存一个不随机器人移动的 6D TCP 目标 | 让移动底盘真正帮助接近远处目标 |
| 4. 验证 DIK/OSC/WBC | 暂时不用 RL，先用确定性控制器跟踪 TCP | 独立检查 FK、Jacobian、坐标系和关节方向 |
...
1. 测量或定义临时的 `gripper_stator -> tcp` 变换；
2. 完成四个轮子的方向和有效半径测试；
3. 实现固定世界坐标 command 及其不变性测试；
4. 建立单环境 DIK/OSC 机械臂 tracking 基线；
5. 注册 `B2W-Z1-TCP` 与 `B2W-Z1-TCP-Play`；
6. 通过 1、16、64 环境测试和 checkpoint 保存/加载测试；
7. 从标称平地的静态目标开始 Stage 1 PPO 训练。

第一轮长训练开始前，至少还需要明确 TCP、轮子方向/半径，以及确认新的任务能
在小规模并行环境中稳定运行。
